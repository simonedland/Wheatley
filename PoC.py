...
###############################################################################
import asyncio, io, logging, random, re, sys, threading, time, textwrap
...
COL = dict(build="red", tts="yellow",
           done="orange", retry="yellow",
           ready="green", play="bright_blue",
           end="magenta", err="bright_red")
...
def set_phase(idx: float, phase: str, txt: Optional[str]=None,
    ...
    if txt is not None:
        r["text"] = txt
    r["phase"] = phase
    r["status"] = status or dict(build="building", tts="→ TTS", retry="RETRY",
                                 done="complete", ready="ready", play="playing",
                                 end="played", err="ERROR")[phase]
    render(rows)
...
def http_tts(text: str, prev: str, nxt: str, idx: float) -> bytes | None:
    ...
    while attempt <= MAX_ATTEMPTS:
        ...
        set_phase(idx, phase,
                  status=f"{'RETRY' if attempt>1 else '→ TTS'} "
                         f"({attempt}/{MAX_ATTEMPTS})")
        try:
            resp = _post(); resp.raise_for_status()
            set_phase(idx, "ready")
            return resp.content
        except Exception as e:
            code = getattr(e.response, "status_code", None)
            body = getattr(e.response, "text", "")[:160] if hasattr(e,"response") else str(e)
            print(textwrap.fill(
            ...
            backoff = min(backoff*2, 32); attempt += 1
...
def gpt_thread(prompt: str, q: asyncio.Queue, loop):
    buf, tmp, last, idx, scan = "", "", time.time(), 0.0, 0
    ...
    for ch in stream:
        ...
        if not USE_RICH and time.time()-last > TOKEN_FLUSH_MS/1000:
            print(tmp, end="", flush=True); tmp=""; last=time.time()
        while True:
            ...
            if word in ABBREVS or re.fullmatch(r'\d+', word):
                scan = m.end(); continue
            sent, buf = buf[:m.end()].strip(), buf[m.end():].lstrip(); scan = 0
            add_row(idx, sent)
            asyncio.run_coroutine_threadsafe(q.put((idx, sent)), loop)
            set_phase(idx, "done"); idx += 1.0
    if buf.strip():
...
async def sequencer(aq: asyncio.Queue):
    loop, heap, expect = asyncio.get_running_loop(), {}, 0.0
    while True:
...
async def chat(prompt: str):
    q_sent, q_audio = asyncio.Queue(QUEUE_MAXSIZE), asyncio.Queue(QUEUE_MAXSIZE)
    ...
    with ThreadPoolExecutor(MAX_TTS_WORKERS) as pool:
        await asyncio.gather(tts_dispatch(q_sent, q_audio, pool),
                             sequencer(q_audio))
